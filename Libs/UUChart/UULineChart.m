...
-(void)setYLabels:(NSArray *)yLabels
{
    ...
    for (int i=0; i<5; i++) {
        if ([_showHorizonLine[i] integerValue]>0) {
            ...
            shapeLayer.strokeColor = [[[UIColor whiteColor] colorWithAlphaComponent:0.0] CGColor];
...
-(void)setXLabels:(NSArray *)xLabels
{
    ...
    for (int i=0; i<xLabels.count; i++) {
        UUChartLabel * label = [[UUChartLabel alloc] initWithFrame:CGRectMake(i * _xLabelWidth+UUYLabelwidth, self.frame.origin.y + 5, _xLabelWidth, UULabelHeight)];
        label.textColor=[UIColor whiteColor];
        label.font = [UIFont systemFontOfSize:12];
        [self addSubview:label];
        
//        weather_weatherModel * model = xLabels[i];
...
//        }
    ...
    //白天天气
    for (int i=0; i<xLabels.count; i++) {
        UUChartLabel * label = [[UUChartLabel alloc] initWithFrame:CGRectMake(i * _xLabelWidth+UUYLabelwidth, self.frame.origin.y + 5 + UULabelHeight, _xLabelWidth, UULabelHeight)];
        label.textColor=[UIColor whiteColor];
        label.font = [UIFont systemFontOfSize:12];
        [self addSubview:label];
        
//        weather_weatherModel * model = xLabels[i];
//         NSString * str = [NSString stringWithFormat:@"%@",model.info.day[1]];
//        label.text = str;
    ...
    //夜间天气
    for (int i=0; i<xLabels.count; i++) {
        UUChartLabel * label = [[UUChartLabel alloc] initWithFrame:CGRectMake(i * _xLabelWidth+UUYLabelwidth, self.frame.size.height - UULabelHeight*3+20, _xLabelWidth, UULabelHeight)];
        label.textColor=[UIColor whiteColor];
        label.font = [UIFont systemFontOfSize:12];
        [self addSubview:label];
    
//        weather_weatherModel * model = xLabels[i];
//        NSString * str = [NSString stringWithFormat:@"%@",model.info.night[1]];
//        label.text = str;
     ...
     //星期
    for (int i=0; i<xLabels.count; i++) {
        UUChartLabel * label = [[UUChartLabel alloc] initWithFrame:CGRectMake(i * _xLabelWidth+UUYLabelwidth, self.frame.size.height - UULabelHeight*2+20, _xLabelWidth, UULabelHeight)];
        label.textColor=[UIColor whiteColor];
        label.font = [UIFont systemFontOfSize:12];
        [self addSubview:label];
        
//        weather_weatherModel * model = xLabels[i];
//        NSString * str = [NSString stringWithFormat:@"周%@",model.week];
//        label.text = str;
...
- (void)addPoint:(CGPoint)point index:(NSInteger)index isShow:(BOOL)isHollow value:(CGFloat)value
{
    ...
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(point.x-UUTagLabelwidth/2.0, point.y-UUWHeight*1, UUTagLabelwidth, UUWHeight)];

    if ( index!=0 ) {
       label.frame = CGRectMake(point.x-UUTagLabelwidth/2.0, point.y-UUWHeight, UUTagLabelwidth, UUWHeight);
